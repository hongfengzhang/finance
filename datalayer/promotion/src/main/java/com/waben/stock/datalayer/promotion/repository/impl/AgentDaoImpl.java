package com.waben.stock.datalayer.promotion.repository.impl;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.PageRequest;
import org.springframework.data.domain.Pageable;
import org.springframework.data.jpa.domain.Specification;
import org.springframework.stereotype.Repository;

import com.waben.stock.datalayer.promotion.entity.Agent;
import com.waben.stock.datalayer.promotion.repository.AgentDao;
import com.waben.stock.datalayer.promotion.repository.impl.jpa.AgentRepository;

/**
 * 经纪人 Dao实现
 * 
 * @author luomengan
 *
 */
@Repository
public class AgentDaoImpl implements AgentDao {

	@Autowired
	private AgentRepository repository;

	@Override
	public Agent create(Agent t) {
		return repository.save(t);
	}

	@Override
	public void delete(Long id) {
		repository.delete(id);
	}

	@Override
	public Agent update(Agent t) {
		return repository.save(t);
	}

	@Override
	public Agent retrieve(Long id) {
		return repository.findById(id);
	}

	@Override
	public Page<Agent> page(int page, int limit) {
		return repository.findAll(new PageRequest(page, limit));
	}

	@Override
	public Page<Agent> page(Specification<Agent> specification, Pageable pageable) {
		return repository.findAll(specification, pageable);
	}

	@Override
	public List<Agent> list() {
		return repository.findAll();
	}

}
