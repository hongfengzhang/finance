...
import com.waben.stock.datalayer.promotion.business.StockOptionCycleBusiness;
import com.waben.stock.datalayer.promotion.business.StrategyTypeBusiness;
...
public class BenefitConfigService {
	...
	public List<BenefitConfigBean> benefitConfigList(Long orgId, Integer resourceType) {
		List<BenefitConfigBean> result = new ArrayList<>();
		...
		if (resourceType == 1) {
			...
			if (strategyTypeList != null && strategyTypeList.size() > 0) {
				...
				for (StrategyTypeDto strategyType : strategyTypeList) {
					BenefitConfigBean configBean = new BenefitConfigBean();
