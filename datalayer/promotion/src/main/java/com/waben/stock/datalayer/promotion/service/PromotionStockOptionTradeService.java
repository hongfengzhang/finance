...
public class PromotionStockOptionTradeService {
	...
	public Page<PromotionStockOptionTradeBean> pagesByQuery(PromotionStockOptionTradeQuery query) {
		...
		String sql = String
				.format("select t1.id, t1.publisher_id, t1.publisher_phone, t1.stock_code, t1.stock_name, t1.cycle_name, "
						...
						+ "LEFT JOIN p_organization t3 on t3.code=t2.org_code where 1=1 %s %s %s %s limit " + query.getPage() * query.getSize() + "," + query.getSize(), query.getCurrentOrgCode(), tradeIdCondition, stateCondition, publisherPhoneCondition, orgCodeCondition);
		...
		List<PromotionStockOptionTradeBean> content = dynamicQuerySqlDao.execute(PromotionStockOptionTradeBean.class, sql, setMethodMap);
		BigInteger totalElements = dynamicQuerySqlDao.executeComputeSql(countSql);
		// 获取股票的最新价
		List<String> codes = new ArrayList<>();
		for (PromotionStockOptionTradeBean trade : content) {
		...
		if (codes.size() > 0) {
			...
			if (stockMarketList != null && stockMarketList.size() > 0) {
				for (int i = 0; i < stockMarketList.size(); i++) {
					...
					PromotionStockOptionTradeBean record = content.get(i);
